...
Implement a change's tasks one slice at a time, branching by schema — ready for `tospec-archive` once the tasks are done and the tests are green. The two-axis review is opt-in: it runs only if the user asks for it.
...
### sdd branch — implement per slice
